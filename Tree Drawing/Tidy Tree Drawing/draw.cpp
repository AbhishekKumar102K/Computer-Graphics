...
void Draw::Draw_Circle(int x, int y, int xoff, int yoff)
{
...
void Draw::draw_line1(int x1, int y1, int x2, int y2)
{
	int k = y2 > y1 ? 1 : -1;
	int dx = x2 - x1;
	int dy = abs(y2 - y1);
...
void Draw::draw_line2(int x1, int y1, int x2, int y2)
{
	int k = x2 > x1 ? 1 : -1;
	int dx = abs(x2 - x1);
	int dy = y2 - y1;
...
void Draw::printCircle(int xoff, int yoff, int r)
{
	int radius = r;
	int x=0;
	int y=radius;
